...
## Осознанные трактовки статьи Ochs (расхождения зафиксированы, не пересматривать без обсуждения)

- **Маршрутизация социального эффекта эмоции — строго по causeId = evt.AgentId** (раунд правок 2, задача 1, `Simulation/SocialDynamicsEngine.cs`, `Perceive`; буквальное прочтение Fig. 4-6). Каждая эмоция, порождённая событием, несёт единственную мишень социального эффекта — причину (agent), никогда не пациента. Если causeId == perceiverId (самопричинённая эмоция — я сам совершил действие), социальный эффект отбрасывается целиком: рёбер «к себе» не существует, и никакого fallback на пациента или иного участника нет. До этого раунда правок fallback-логика перенаправляла самопричинённый эффект на пациента, из-за чего perceiver, причинивший вред кому-то и испытавший от этого distress, ошибочно ПОНИЖАЛ liking к пострадавшему («обвинение жертвы») — прямо противоречит статье (ср. сцену допроса: distress грабителя от воспоминания о похитителях не меняет liking к полицейскому, т.к. «the policeman is not responsible»). Эмоция при этом всё равно остаётся в EmotionVector NPC — отбрасывается только её вклад в SocialRelation. Это правило поглощает прежнюю формулировку про "третью сторону-наблюдателя" (counterparty=evt.AgentId для perceiver, не являющегося ни агентом, ни пациентом) — та ветка уже была частным случаем этого же принципа и осталась без изменений.
- **Fortunes-of-others (happy-for/sorry-for/gloating/resentment) исключены** вслед за разделом III-C статьи — она рассматривает их отдельно от базовой модели, реализуемой здесь. Наблюдатель (не агент и не пациент события) не формирует отношения к пациенту события — только к причине (agent), если он вообще воспринимает событие. Нулевое отношение наблюдателя к пациенту — ожидаемое поведение по текущей архитектуре, не баг. Возможное расширение — отдельным решением в будущем, не в этом раунде.
- **Открытый вопрос IV-B (dominance грабителя в диаде с полицейским, причина — третье лицо-похититель) остаётся открытым** и формальным правилам маршрутизации по causeId не подчиняется: у реплики о похищении в тексте статьи нет формального события ⟨agent, action, patient, dc⟩, обе разумные интерпретации (agent=полицейский или agent=похититель) конфликтуют с одной из половин текстового описания. Решение отложено до введения понятия «разговорный контекст» (отдельный будущий этап), см. также «Известный риск» историю выше по WideEdge для примера того, как подобные пробелы закрывались раньше.
- **Counterparty при третьей стороне-наблюдателе** (см. `Simulation/SocialDynamicsEngine.cs`, `Perceive`): статья рассматривает только диадические примеры agent/patient, поэтому для события с наблюдателем, не участвующим напрямую, причина (causeId) берётся как `evt.AgentId` — наблюдатель формирует мнение о том, кто совершил действие, а не о том, на кого оно было направлено. Собственное расширение, не прямая цитата статьи; согласуется с общим правилом маршрутизации по causeId выше.
- **Familiarity следует III-D, а не буквальному IV-A** (задача 4, раунд правок 2026-07-06): статья противоречит сама себе — раздел III-D говорит, что эмоции не имеют прямого влияния на familiarity (растёт только через передачу конфиденциальной информации, механизм вне статьи), раздел IV-A буквально прочитанный предполагает, что f_sr увеличивает «каждое измерение», включая familiarity. Выбрано следование III-D: эмоции (`SocialRelationRules.Apply`) на familiarity не влияют вообще, ни прямо, ни косвенно через liking (прежний `UpdateFamiliarityFromLikingShift`, вызывавший дрейф ~0.01 за событие, удалён). Единственный канал изменения familiarity — явная передача информации через задел `SocialRelationRules.FamiliarityFromInformationTransfer(current, confidentiality)` (пока не подключён к движку) и будущий decay по §6.2.4 диссертации О'Коннора.

## Раунд правок 1 по итогам ручного тестирования UI (2026-07-06, `TASK_fixes_round1.md`)

Ручной прогон UI выявил, что эмоции и отношения оставались нулевыми при событиях с `patientId != perceiverId` — первопричина не в формулах (они уже были верны и покрыты тестами), а в том, что редактор персонажей позволял задать только «Attitude к себе», а `Appraisal`/`ActionDictionary` уже поддерживали произвольные attitude/praise на уровне ядра (Этап 2), просто не были прокинуты в UI.

1. **Attitudes к произвольным сущностям** — `Appraisal` уже хранил `Dictionary<int,float>` (ядро менять не пришлось). Добавлена вкладка «Субъективные оценки»: `AttitudeRow` (`RelationshipSimulator/AttitudeRow.cs`) — строка с редактируемыми `OwnerId`/`TargetEntityId`/`Attitude`, ищет `NpcState` через `SocialDynamicsEngine.GetState` по `OwnerId` в момент чтения/записи (не держит захваченную ссылку, т.к. Owner тоже редактируемый).
2. **Praiseworthiness действий** — `Appraisal.GetPraise`/`SetPraise` тоже уже существовали (субъективно, не в `ActionDictionary` — см. «Архитектура объединения слоёв», п.4). Добавлен `PraiseRow` и та же вкладка, вторая таблица (Владелец/ActionId/Praise).
3. **Валидация диапазонов на вводе** — `RelationshipSimulator/RangeValidationRule.cs` (`ValidationRule`, `ValidationStep=ConvertedProposedValue`, срабатывает ДО записи в источник привязки, не молчаливый кламп) навешан на все численные колонки (`Extraversion`/`Neuroticism`/`SelfAttitude`/`Effect`/`Attitude`/`Praise` → `[-1,1]`; `dc` → `[0,1]`; `Время` → `[0,∞)`). `ValidatingTextBoxStyle` (ресурс окна) подсвечивает ячейку при `Validation.HasError`. Подтверждено вживую: ввод `dc=5.00` не проходит в модель, `DataGrid` показывает стандартный красный индикатор ошибки строки, ячейка возвращается к последнему валидному значению.
4. **Familiarity** — см. «Осознанные трактовки статьи Ochs» выше.

**Найден и исправлен реальный баг WPF `DataGrid`, не связанный с моделью** (важно для любого будущего редактора с «перекрёстными» ячейками, где одна колонка вычисляется по данным других колонок той же строки — как `Attitude`, зависящий от `OwnerId`+`TargetEntityId`): если отредактировать `TargetEntityId`, а затем сразу `Attitude` на той же вновь добавленной строке и уйти со вкладки (даже обычным кликом мыши по заголовку другой вкладки — проверено через реальный `SendInput`-клик, не только через `UIA SelectionItemPattern`, так что это не артефакт автоматизации), WPF может не закрыть транзакцию редактирования строки полностью, и второе значение (`Attitude`) не долетает до модели — что выглядит один в один как симптом «attitude не долетает до NpcState» из этой задачи, но им не является: сначала было доказано, что ядро/данные корректны (независимая вторая строка с тем же ключом `(OwnerId, TargetEntityId)` всегда читает истинное значение), а сам баг — в незакрытой транзакции редактирования строки `DataGrid`. Исправлено обработчиком `CellEditEnding` на `AttitudeGrid`/`PraiseGrid` (`MainWindow.xaml.cs`, `CrossFieldGrid_CellEditEnding`): после **каждой** ячейки форсируется `grid.CommitEdit(DataGridEditingUnit.Row, true)` через `Dispatcher.BeginInvoke` (не синхронно — иначе реентрантный вызов изнутри `CellEditEnding`), не дожидаясь ухода со строки/вкладки. Подтверждено: без фикса реальный клик мышью по вкладке терял второе значение; с фиксом — сценарий «perceiver=1, agent=2, action(effect=0.7)=1, patient=2, attitude₁(2)=0.8» дал `Joy=0.75`, `liking=0.23`, что совпадает с ручным расчётом формулы.

120 тестов в решении (110 → +10: заморозка familiarity, задел `FamiliarityFromInformationTransfer`, attitude/praise к произвольной сущности через движок), все проходят (`dotnet test`). UI-слой (`AttitudeRow`/`PraiseRow`/`RangeValidationRule`/фикс `CellEditEnding`) не покрыт `RelationshipCore.Tests` (это UI-проект) — проверен вручную через `System.Windows.Automation` и реальные `SendInput`-клики, включая скриншоты для подтверждения (см. выше про баг `DataGrid`, где чтение через `ValuePattern.Current.Value` само по себе периодически возвращало устаревший текст — визуальная проверка скриншотом была необходима, автоматизации одной было недостаточно).

## Раунд правок 2 по итогам ручного тестирования UI (2026-07-07, `TASK_fixes_round2.md`)

Порядок выполнения по заданию: 2 → 1 → 3 → 4.

1. **Задача 2 (утечка praise) — заявленный баг не воспроизвёлся.** Диагностика в два независимых прохода: (а) прямой вызов `SocialDynamicsEngine.Perceive` с эталонным сценарием задачи в throwaway xUnit-тесте, (б) полный живой прогон того же сценария через `System.Windows.Automation` (свежепостроенный harness-проект, `GridPattern.GetItem(row,col)` + `ValuePattern.SetValue` — та же техника, что и в раунде 1, оформленная в отдельный переиспользуемый скрипт, не оставленный в репозитории). Оба прохода дали `Admiration₃ = 0` (не 0.20, как в описании задачи) — `Appraisal` уже хранит `_praises` как приватный `Dictionary<int,float>` per-instance на каждый `NpcState`, отдельный от `_attitudes` и от `_praises` других владельцев; промах по ключу отдаёт 0 без какого-либо fallback. Вывод: лишнего lookup в `Simulation`/`ActionDictionary` нет и не было — вероятнее всего, "утечка" в исходном ручном прогоне была случайной опечаткой при ручном заполнении `PraiseGrid` (например, значение введено в не ту ещё-не-переключённую по Owner/Action строку), а не системным дефектом. Задокументировано как regression-гарантия, а не как исправление: `RelationshipCore.Tests/Dynamics/AppraisalTests.cs` (4 теста — промах по действию, промах по несуществующей записи, изоляция между владельцами для praise и для attitude).
2. **Задача 1 (маршрутизация эффектов эмоций) — реальный баг, исправлен.** См. «Осознанные трактовки статьи Ochs» выше (пункт про causeId). `SocialDynamicsEngine.Perceive` — убран `counterparty`-fallback (`perceiverId == evt.AgentId ? evt.PatientId : evt.AgentId`), заменён на явный `causeId = evt.AgentId` с условием `if (causeId != perceiverId) UpdateRelation(...)`. Интеграционный тест `RelationshipCore.Tests/Simulation/EmotionRoutingScenarioTests.cs` (4 теста) воспроизводит ровно эталонный сценарий из задачи (3 персонажа, 4 события, все `dc=1, t=0`) и сверен и через xUnit, и через тот же живой UI-harness: `1→2` (самопричинённый вред) — ребро вообще не создаётся; `2→1`, `3→2` — каналы, не задействованные багом, продолжают работать как раньше; `3→1` — fortunes-of-others исключены, отношение нулевое. Числа в самой задаче (`≈-0.29`, `≈-0.12`, `≈+0.34`) были прикидкой автора задачи вручную — исправленный движок даёт `-0.08`/`+0.23` для 2→1/3→2 (совпадает по знаку и порядку величины; точные числа в тесте — не переписанные вручную, а взятые из фактического вывода исправленного кода).
3. **Задача 3 (единый источник правды для attitude «к себе»)** — колонка «Attitude к себе» на вкладке «Персонажи» удалена (`NpcRow.SelfAttitude` убран, `MainWindow.xaml` — колонка убрана из `NpcGrid`, добавлена подсказка «задаётся на вкладке Субъективные оценки»); единственное хранилище — таблица `Attitudes` (строка `OwnerId == TargetEntityId` — это и есть self-attitude), она и раньше писала в тот же `Appraisal`, так что риск рассинхронизации устранён не слиянием двух хранилищ (их и не было двух на уровне данных — они разошлись бы только на уровне двух независимых UI-виджетов), а удалением второго виджета.
   - **Запрет дублей `(OwnerId, TargetEntityId)`**: `AttitudeRow` принимает опциональный `Func<AttitudeRow,int,int,bool> isDuplicate`; сеттеры `OwnerId`/`TargetEntityId` перед применением значения спрашивают `MainWindow.AttitudeWouldDuplicate` — если пара уже занята другой строкой, правка отклоняется (UI откатывается на прежнее значение через принудительный `OnPropertyChanged`).
   - **Найдена и исправлена вторая проблема при живой проверке**, не описанная в задаче явно: первая реализация `AddAttitude_Click` заготавливала новую строку с фиксированным дефолтом `owner = target = Npcs[0].EntityId` — но именно эту пару требовала защита от дублей проверять НА КАЖДЫЙ клик, а не только первый. Второй клик "Добавить attitude" до того, как пользователь успевал отредактировать первую строку, обнаруживал, что пара `(1,1)` уже занята, и — в первой версии фикса — тихо переиспользовал существующую строку вместо создания новой, что на практике означало: после создания одной "самоотношения"-строки НИ ОДНУ новую строку добавить кнопкой было нельзя, пока не увести первую от дефолта. Обнаружено вручную через тот же UI-harness (клик × 6 создавал куда меньше 6 строк). Исправлено: новая строка получает заведомо свободный `TargetEntityId` (`Math.Min(-1, Attitudes.Min(r => r.TargetEntityId) - 1)` — на единицу меньше минимального среди уже существующих, гарантированно никогда не совпадёт ни с одной реальной парой), так что `AttitudeWouldDuplicate` на самом Add никогда не срабатывает — защита от дублей остаётся активной только там, где она и нужна: при ручном редактировании `OwnerId`/`TargetEntityId` существующей строки на значение другой существующей строки.
   - Подтверждено вживую тем же harness: попытка увести строку `(1→2)` на `(2→1)` (уже занято другой строкой) отклонена, строка осталась `(1→2)`; полный эталонный сценарий задачи 1 через self-attitude, заданные исключительно через `Attitudes`-таблицу (не через удалённую колонку), дал те же числа, что и прямой вызов движка.
4. **Задача 4** — см. «Осознанные трактовки статьи Ochs» выше (3 новых/обновлённых пункта: маршрутизация по causeId, исключение fortunes-of-others, открытый вопрос IV-B).

129 тестов в решении (120 → +9: 4 `AppraisalTests`, 4 `EmotionRoutingScenarioTests`, 1 `Perceive_SelfCausedHarmToThirdParty_...` в `SocialDynamicsEngineTests`), все проходят (`dotnet test`). UI-слой (`NpcRow`/`AttitudeRow`/`MainWindow.xaml(.cs)`) снова не покрыт `RelationshipCore.Tests` — проверен вручную через `System.Windows.Automation`-harness (тот же приём, что в раунде 1, оформлен в переиспользуемый скрипт для этого раунда, не сохранён в репозитории как временный инструмент диагностики).
